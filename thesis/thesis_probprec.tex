...
Baselines are taken from XXX, well-tuned hyperparameters, but no schedule or such things.
...
A DeepOBS-aided grid search with 10 evaluations on a logarithmic grid between $10^-5$ and $10^2$ on the \verb|fmnist_2c2d| testproblem. The results are demonstrated in \ref{fig:exp_lr_sens}, together with the baseline SGD reference.
...
\section{Experiment aa: Auto-Restart}
...
\section{Discussion}
\begin{markdown}
- Preconditioning: Why does it not help or even make things worse? 
	- It can compress at most 2 of the eigenvalues!
	- It uses less data to train
	- due to the noisyness the Preconditioner might actually worsen the problem
	- the constructed learning rate might be more conservative (smaller) than the empirical optimum, leading to slower convergence
- Performance: Why is the algorithm so slow, could we improve speed?
	- All the computation-intensive linear Algebra happens in `_create_low_rank()` on the CPU in numpy. It could be ported to GPU-ready code.
- Initialization:
	- There might be some problem/initializations that make the algorithm fail. One would need to do a much more exhaustive search or analytical analysis on this.
