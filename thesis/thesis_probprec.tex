...
\done[inline]{run AdaptiveSGD and PreconditionedSGD on 9 more seeds on the problems}
...
\begin{figure}
	...
	\caption{Both variants of the algorithm perform significantly worse than established alternatives. AdaptiveSGD is better than PreconditionedSGD}
...
\done[inline]{Run runtime-estimation for AdaptiveSGD and PreconditionedSGD and OnlyAdaptiveSGD for a few problems}
\done[inline]{Report results: OnlyAdaptiveSGD=SGD < AdaptiveSGD < PreconditionedSGD}
...
\done[inline]{monkey patch an uninitialized testproblem into deepobs}
\done[inline]{run PreconditionedSGD without lr, with and without initialization, on 10 seeds for 5 epochs, only cifar10}
\done[inline]{Batch size 64}
...
\done[inline]{run PreconditionedSGD without lr on cifar10 and fmnist, see preconditioning}
\todo[inline]{rerun best settings 10 times(maybe done, maybe not)}
...
\done[inline]{plot} see fig \ref{fig:exp_lr_sens}.
