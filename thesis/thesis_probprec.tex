...
\todo{formula: Noisy update rule}

\subsection{First-Order and Second-Order Methods}
Depending on our problem, we can use more available information in order to take an even better optimization step. Different optimizers have been proposed that use varying amounts of information.
There are zeroth-order methods that don't directly take the gradient into account at all.
The group to which SGD belongs is the group of First-Order Methods, named because these methods use the first derivative of the loss function. Adaptive Modifications of SGD like Adam and Momentum keep track of past gradients in order to take better steps. This implicitly assumes that past gradients contain information about future gradients, which is often the case. For example, if there is little change in the last gradients, we can assume that we are far away from the minimum, where gradients should converge to zero. Therefore, we are able to take larger steps.
...
\todo[inline]{derivation of why the condition number is importand (Nocedal...)}
...
- The convergence of optimizers depends on the condition number of the Hessian \todo{formula}, which describes the whole loss landscape.
\todo{Image}
...
\section{Probablilty Basics}
- Probability Distribution of a random variable, normal distribution, multivariate Gaussian distribution
- Point estimates
- Bayes' Rule
