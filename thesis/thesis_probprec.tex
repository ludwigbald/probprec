...
\end{markdown}
\done{Could include a runscript with  necessary changes in the appendix.}
...
\end{figure}
\todo[inline]{Preconditioned Tuned SGD is so good! This might mean that the algorithm always gets the learning rate quite wrong. The preconditioning seems to help with a fixed learning rate (on cifar), but it makes things worse for the adaptive learning rate.}
