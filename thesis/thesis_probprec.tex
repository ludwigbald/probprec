...
- SGD is a widely used optimization algorithm. Many variations on SGD have been proposed and sometimes are quicker, but it's hard to study optimizers.
...
- Second order methods seem to be crowd's favourites now. They tend to learn quicker as they use more information to find the minimum.
\end{markdown}
...
\todo{Connect to sgd. Original algorithm finds single alpha which is the same as normal sgd. Parameter groups allow alpha\_l per layer. \\
	Convergence statement not as obvious. It depends on many factors but having a more adaptive optimizer could be beneficial.}
...
\end{markdown}
\todo[inline]{Make clearer the optimizer input, this is a large contribution that I only mentioned in my paper but never implemented. 
	Maybe include the Preconditioner class definition as a verbatim environment and discuss the inputs.
	And maybe also a pseudo code for the algorithm.}
...
\done{Describe the models/architectures used for the datasets.}
...
DeepOBS comes with results from many established and well-studied optimizers as a baseline, and it is easy to compare a new optimizer against them. Their hyperparameters are tuned for each testproblem, but some standard techniques like hyperparameter schedules are not available.

\done[inline]{Mention that one advantage of a package like DeepOBS offers baselines for various metrics. 
	What kind of baselines are used? Optimizers, optimization curves, hyperparameters?}
\section{Technical details}
The experiments were run on the TCML cluster at the University of Tübingen.
A Singularity container was set up on Ubuntu 16.4 LTS with python 3.5, pytorch (version) and DeepOBS (see Appendix for Singulariy recipe\done{append recipe}).
...
\done{Include specific hardware details}
...
\todo[inline]{make clear we're measuring wallclock time here. CPU time would also be interesting}
\todo[inline]{Mention that CPU time would also be interesting. Maybe as a proposition for DeepOBS?}
\begin{markdown}
...
\todo{Talk about new version using bs 32, explain results}
\begin{figure}
	\centering \hspace{-1,5cm}
...
\todo[inline]{Clarify: So estimating the Hessian with B=128 costs the same as 2 epochs of sgd? Or is it referring to applying the preconditioner over an epoch. The latter makes sense but can be remedied and is why it should rather be compared to adam in cost. --> DeepOBS paper?}
\todo[inline]{RUNNING: add Adam in the comparison}
