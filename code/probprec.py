...
class Preconditioner(torch.optim.Optimizer):
    ...
    def __init__(self, params, est_rank=2, num_observations=5, prior_iterations = 10, weight_decay = 0, lr = None, 
        ...
        self.stepnumber = 0
        self.lam=torch.zeros(1)       
        self.W_var=torch.zeros(1) 
    ...
    def _InitializeLists(self):
        for group in self.param_groups:
            self.accumulated_hess_vec=list(torch.zeros_like(p) for p in group['params'])
            
            self.gradient=list(torch.zeros_like(p) for p in group['params'])
            self.accumulated_gradient=list(torch.zeros_like(p) for p in group['params'])
            self.vec=list(torch.ones_like(p) for p in group['params'])
            self.STAS=list(torch.zeros(1) for p in group['params']) 
            self.STAAS=list(torch.zeros(1) for p in group['params'])
            self.STS=list(torch.zeros(1) for p in group['params'])  

            
            self.S=list( torch.zeros((tuple(p.size())+(self.num_observations,))) for p in group['params'])
            self.X=list( torch.zeros((p.numel(),self.num_observations)) for p in group['params'])
            self.Y=list( torch.zeros((tuple(p.size())+(self.num_observations,))) for p in group['params'])
            
            self.STWS=list( torch.zeros((self.num_observations,self.num_observations)) for p in group['params'])
            self.STLS=list( torch.zeros((self.num_observations,)) for p in group['params'])
            self.inner_product=list( torch.zeros((self.num_observations,self.num_observations)) for p in group['params'])
    ...
    def EstimatePrior(self):
            ...
            print('[Estimate prior] alpha: {:.2e} w: {:.2e} lambda: {:.2e}'.format(self.alpha.item(),self.W_var.item(),self.lam.item()))
    ...
    def GatherCurvatureInformation(self):
        df_sum=torch.zeros(1)#tensor([0.0])
...
#                 vec=alph*w_var
#                 print(X.shape)
                ip[0,0] = alph*w_var**2*torch.sum(X[:,0]*S[...,0].view(-1))
...
#                 print(S.shape,torch.norm(vec))
                ...
                delta=(Y[...,:m+1]-alph*S[...,:m+1]).view(-1,m+1)
#                 print(delta.shape,delta[0,:])
                ...
                STWS.data[:m,m]=w_var*torch.mv(S[...,:m].view(-1,m).t(),S[...,m].view(-1))
                ...
                D,V=torch.symeig((STWS[:m+1,:m+1]/stls)/stls.unsqueeze(1),eigenvectors=True)
                # print(D.data)
#                 print('V: ',V,D)
#                 torch.symeig()
...
#                 print(torch.mm(delta[:2,:],F_V)/torch.sqrt(lam))
                X.data[:,:m+1]=torch.mm((torch.mm(delta,F_V)/torch.sqrt(lam))/(w_var/lam*D+1.0),F_V.t())/torch.sqrt(lam)
#                 print('X: ',X[0:2,:])
#                 X=delta/(w_var*STWS[0,0]+lam*STLS[0])
#                 print(torch.sum(S[...,0]*X*w_var**2 +1))

#                 print(X.shape)
                ip[:m+1,:m+1] = alph*w_var**2*torch.mm(X[:,:m+1].t(),S[...,:m+1].view(-1,m+1))
        ...
        for group in self.param_groups:
            ...
            for p,S,X in zip(group['params'],self.S,self.X):
                V=S.view(-1,self.update_counter)
                    # print('Create Low Rank')
                    # print(V.shape)
                    # print(torch.matmul(V.t(),V))
                def Matv(v):
                    return w_var.numpy()**2*X.numpy().dot(S[...,:self.update_counter].view(-1,self.update_counter).numpy().T.dot(v)) + v/alph.numpy()

                def rMatv(v):
                    return w_var.numpy()**2*S[...,:self.update_counter].view(-1,self.update_counter).numpy().dot(X.numpy().T.dot(v)) + v/alph.numpy()


                effective_rank=np.min([self.rank,X.shape[0]-1])
                LinOp = splinalg.LinearOperator((X.shape[0],X.shape[0]),matvec=Matv,rmatvec=rMatv)
                sing_vec,sing_val,_ = splinalg.svds(LinOp,k=effective_rank)#,tol=1e-2)
                print(' 1/a: {:.2e} w: {:.2e} sigma: {}'.format(1.0/alph.item(),w_var.item(),np.sqrt(sing_val)))
                ...
                state = self.state[p]
                state['preconditioned_vectors']=torch.from_numpy(sing_vec[:,:effective_rank].astype(np.float32)).view(shape) #.view_as()
                state['preconditioned_scaling']=torch.from_numpy(np.sqrt(sing_val[:effective_rank]).astype(np.float32))
        ...
        if self.stepnumber < self.prior_iterations:
            ...
            for group in self.param_groups:
                for p in group['params']:
                    ...
                    if grad.is_sparse:
                        raise RuntimeError('Method does not support sparse gradients')
                    state = self.state[p]
                    ...
                    buf=torch.sum(U * (1.0/D - 1.0) * torch.mv(U.view(-1, effective_rank).t(), grad.view(-1)), dim=-1) #conditioned gradient
