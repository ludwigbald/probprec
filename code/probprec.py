...
class Preconditioner(torch.optim.Optimizer):
    ...
    def _InitializeLists(self):
        for group in self.param_groups:
            ...
            self.STAS=list(torch.zeros(1, device = self.device) for p in group['params'])
            self.STAAS=list(torch.zeros(1, device = self.device) for p in group['params'])
            self.STS=list(torch.zeros(1, device = self.device) for p in group['params'])


            self.S=list( torch.zeros((tuple(p.size())+(self.num_observations,)), device = self.device) for p in group['params'])
            self.X=list( torch.zeros((p.numel(),self.num_observations), device = self.device) for p in group['params'])
            self.Y=list( torch.zeros((tuple(p.size())+(self.num_observations,)), device = self.device) for p in group['params'])

            self.STWS=list( torch.zeros((self.num_observations,self.num_observations), device = self.device) for p in group['params'])
            self.STLS=list( torch.zeros((self.num_observations,), device = self.device) for p in group['params'])
            self.inner_product=list( torch.zeros((self.num_observations,self.num_observations), device = self.device) for p in group['params'])
    ...
    def CreateLowRank(self):
        ...
        for group in self.param_groups:
            ...
            for p,S,X in zip(group['params'],self.S,self.X):
                ...
                state = self.state[p]
                state['preconditioned_vectors']=torch.from_numpy(sing_vec[:,:effective_rank].astype(np.float32)).view(shape).to(self.device) #.view_as()
                state['preconditioned_scaling']=torch.from_numpy(np.sqrt(sing_val[:effective_rank]).astype(np.float32)).to(self.device)
